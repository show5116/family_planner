...
```json
{
  ...
  "createdAt": "2025-01-01T00:00:00Z", // 생성일시 (Date)
  "balanceAfter": "4500000.00", // 출금 후 잔액 (직전 스냅샷 기준, 스냅샷 없으면 null) (string | null)
  "principalAfter": "4300000.00", // 출금 후 원금 (직전 스냅샷 기준, 스냅샷 없으면 null) (string | null)
  "profitAfter": "200000.00", // 출금 후 수익금 (직전 스냅샷 기준, 스냅샷 없으면 null) (string | null)
  "profitRate": "4.17" // 출금 후 수익률 (%, 직전 스냅샷 기준, 스냅샷 없으면 null) (string | null)
}
```
...
### GET `assets/groups/:groupId/holding-records/names`

**요약:** 그룹 종목명 목록 조회 (자동완성용)
...
```json
{
  ...
  "unallocatedAmount": "500000.00" // 미할당 금액 (잔액 - 종목 합계, 음수면 초과) (string)
}
```
...
```json
{
  ...
  "startDate": "2026-03-01", // 시작 월 (YYYY-MM-DD, 일자는 무시되고 1일로 저장됨). 과거 월을 지정하면 등록 시점에 소급하여 오늘까지 지출 내역을 생성합니다. (string?)
  "totalMonths": 24, // 총 반복 개월 수 (예: 24개월 할부). 생략 시 무기한 반복됩니다. 입력 시 startDate가 함께 필요합니다. (number?)
...
```json
{
  ...
  "dayOfMonth": 25, // 매달 발생 일(day). 1~31 (number)
  "startDate": "2026-03-01T00:00:00.000Z", // 시작 월 (소급 생성 및 종료 시점 계산 기준) (Date | null)
  "totalMonths": 24, // 총 반복 개월 수 (null이면 무기한 반복) (number | null)
  "endDate": "2028-02-01T00:00:00.000Z", // 종료 예정 월 (startDate + totalMonths 기준 계산값, 무기한이면 null) (Date | null)
...
```json
{
  ...
  "startDate": "", // 시작 월 (YYYY-MM-DD, 일자는 무시되고 1일로 저장됨). 수정 시에는 소급 생성을 다시 수행하지 않으며, 종료 시점 재계산에만 사용됩니다. (string?)
  "totalMonths": null, // 총 반복 개월 수 (null 전달 시 무기한 반복으로 변경) (number | null?)
