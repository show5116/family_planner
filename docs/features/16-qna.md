...
## 알려진 문제 (2026-09-08 매뉴얼 작성 중 확인)
- ⚠️ **질문 작성 화면 안내 문구가 실제 규칙과 다릅니다.**
  화면에는 `대기 중 상태에서만 수정/삭제 가능합니다`라고 적혀 있지만 서버 규칙은
  - 수정: 본인 + **RESOLVED가 아니면 가능**. ANSWERED에서 수정하면 PENDING으로
    되돌아갑니다(재질문) — `qna.service.ts:373-379`
  - 삭제: 본인이면 **상태와 무관하게 가능** — `qna.service.ts:397-408`
  앱의 수정 차단도 RESOLVED일 때만 동작합니다(`question_detail_screen.dart:85`).
  안내 문구를 실제 규칙에 맞게 고치는 편이 좋습니다.
