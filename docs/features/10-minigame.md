...
- ✅ 미니게임 메인 화면 (MiniGamesScreen) — 더보기 탭에서 접근
  ...
  - 게임 이력 섹션 (같은 화면 내 인라인) — 최근순 목록 + 새로고침
    - ⚠️ 종류별 필터는 `minigameTypeFilterProvider`와 API 파라미터까지 있으나
      **화면에 노출된 UI가 없습니다** (항상 전체 조회)
...
## 룰렛 돌리기
- ✅ 항목 추가/삭제 UI (기본 3칸으로 시작, 2개 이상 입력해야 돌릴 수 있음)
- ✅ 항목별 비율(weight) — 원판 넓이에 반영
- ✅ 룰렛 회전 애니메이션
- ✅ 결과 표시 애니메이션
- ✅ 항목별 색상 설정
...
- ✅ 게임 이력 조회 (그룹별) — 종류별 필터는 provider·API만 있고 UI 미노출
- ✅ 게임 이력 삭제
