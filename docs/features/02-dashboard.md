...
- ✅ 인사말 섹션 — 사용자가 등록한 "오늘의 한마디" (문구가 없으면 시간대별 메시지로 폴백)
  - 기본 제공 팩 14종을 개별로 켜고 끄기 (총 296문구 × 4개 국어)
    - 아이 나이대별: 아기(0~12개월) 20 · 걸음마(1~3세) 24 · 유아(3~5세) 20 ·
      초등(6~12세) 20 · 청소년(13세~) 20
    - 마음 챙기기: 부모 마음 20 · 명언과 속담 24 · 응원 한마디 24
    - 집안일: 주방과 식품 22 · 세탁과 옷 20 · 청소와 곰팡이 20
    - 영어 한 문장: 생활 회화 22 · 여행과 외식 20 · 직장과 이메일 20
      (문장 뒤에 뜻을 붙이고, 영어 로케일에서는 뜻 대신 쓰는 상황을 붙임)
    - 출처: CDC, AAP HealthyChildren.org, USDA FSIS/FoodSafety.gov,
      American Cleaning Institute, EPA, 아동권리보장원의 공개 가이드를 한 줄로 요약
    - 문구 본문은 ARB가 아니라 `lib/core/constants/greeting_presets/`의
      언어별 상수 파일에 둔다 (1,184개 문자열을 ARB에 넣으면 생성 코드가 과도하게 커짐)
...
- ✅ **이번 주 뷰** (`routineOverviewProvider(period: week)`)
  ...
  - 달성률 % + 일일 목표 달성 일수
  - 다음 배지까지 남은 양 — 판정 기준 3종(연속 달성일·누적 달성일·퍼펙트위크)의 현재값이
    모두 `daily-streak`에 있어 배지 카탈로그 전체가 대상. 단위가 다른 기준끼리는 일수로
    환산해 가장 가까운 것을 고르고 표시만 원래 단위(일/주)로 한다 (조회 실패 시 조용히 생략)
