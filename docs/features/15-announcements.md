...
- ✅ 마크다운 렌더링 지원 (flutter_markdown 패키지 사용)
- ✅ 자동 읽음 처리 (화면 진입 시)
- ✅ 읽은 사람 수 표시

### 관리자 전용 화면 (ADMIN만 접근)
- ✅ 공지사항 작성 화면
  - ✅ 제목, 내용 입력 (마크다운 텍스트 입력)
  - ✅ 고정 여부 토글
  - ⬜ 첨부파일 업로드 (추후 구현)
  - ✅ 미리보기 기능 (마크다운 렌더링)
- ✅ 공지사항 수정 화면
...
### 홈 화면 연동
- ⬜ 새 공지 알림 뱃지 (읽지 않은 공지 개수) - 추후 구현
- ⬜ 고정 공지 미리보기 카드 (옵션) - 추후 구현
...
- ✅ `/announcements/create` - 공지사항 작성 (ADMIN)
- ✅ `/announcements/:id/edit` - 공지사항 수정 (ADMIN)
...
### Phase 1: 기본 조회 ✅
1. ✅ AnnouncementModel 정의 (Freezed 사용)
2. ✅ AnnouncementRepository 구현 (조회 API)
3. ✅ AnnouncementListProvider 구현 (무한 스크롤 지원)
4. ✅ 공지사항 목록 화면 UI
5. ✅ 공지사항 상세 화면 UI
...
### Phase 3: 관리자 기능 ✅
1. ✅ ADMIN 권한 확인 로직 (isAdminProvider)
2. ✅ 공지사항 작성 화면
3. ✅ 마크다운 에디터 통합 (공통 위젯으로 구현)
4. ⬜ 첨부파일 업로드 (추후 구현)
5. ✅ 공지사항 수정/삭제 기능
6. ✅ 고정/해제 토글
