...
- **마크다운 가져오기**: 마크다운 원문을 붙여넣어 서식이 적용된 내용으로 변환 (릴리즈 노트 작성용)
...
- ✅ 공지사항 작성 화면
  ...
  - 고정 여부 토글
  - WYSIWYG 에디터 (굵게, 기울임, 밑줄, 취소선, 인라인 코드, 링크 등)
  - 글자 색상 / 배경 색상 / 글자 크기 지정
  - 마크다운 가져오기 (MD 원문 붙여넣기 → 서식 변환)
- ✅ 공지사항 수정 화면
- ✅ 공지사항 삭제 확인 다이얼로그
- ✅ 고정/해제 토글 버튼
...
### 2026-08-31
- ✅ 에디터 서식 확장: 글자 색상, 배경 색상, 글자 크기, 인라인 코드 버튼 활성화
- ✅ RichTextViewer에 u/s/code/blockquote 스타일 추가 (상세 화면 렌더링 대응)
- ✅ 마크다운 가져오기 기능 추가 (에디터 툴바 버튼)
- ✅ 마크다운 원문 붙여넣기 → HTML → Delta 변환 (저장 포맷은 HTML 유지)
- ✅ 클립보드가 마크다운으로 보이면 다이얼로그에 자동 미리 채움
- ✅ 기존 내용 대체 / 커서 위치 삽입 선택 지원
- ✅ 다국어 지원 (한/영/일/중)
