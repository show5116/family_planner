...
- [x] 현재 플랜 카드에 **남은 기간(`daysLeft`)** 과 상태별 날짜 라벨 — 체험 종료일 / 이용 기간 종료일 / 만료일.
  같은 `expiresAt`이라도 자동 갱신 중인 구독자에게 "만료일"로만 쓰면 서비스가 끊기는 날로 읽힌다.
  자동 갱신 여부(`autoRenewing`)에 따라 **다음 갱신일 / 이용 기간 종료일**로 갈라 표기하고,
  해지된 구독은 경고색으로 "이 날짜에 종료됩니다"를 덧붙인다. 서버가 필드를 안 주면(구버전)
  "해지하지 않으면 갱신됩니다" 조건절로 폴백 → [요청서](../api-proposals/2026-09-04-trial-carryover.md)
...
- **체험 중(`isTrial`)에도 구매 버튼 노출** — 무료 체험은 tier가 `ad_free`라, tier만 보고 버튼을 숨기면
  체험자가 체험 종료 전에 결제할 길이 없다. 애플이 결제 플로우를 확인 못 해 v1.2.0 (8)이 반려된 원인
  (Guideline 2.1). 잔여 체험일 이월은 백엔드 몫 → [요청서](../api-proposals/2026-09-04-trial-carryover.md)
