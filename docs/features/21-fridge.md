...
- ✅ 유통기한 프리셋 관리 화면 (ExpiryPresetManagementScreen)
...
├── data/
│   ├── models/fridge_models.dart          ← StorageModel, FridgeItemModel,
│   │                                         ExpirysuggestionModel, ExpiryPresetModel,
│   │                                         장바구니·자주 사는 것·구매 이력 모델
│   └── repositories/fridge_repository.dart  ← 냉장고 + 장보기 API 전체
├── providers/fridge_provider.dart          ← storagesWithItemsProvider,
│                                              fridgeSelectedGroupIdProvider, expiryPresetsProvider
└── presentation/
    ├── screens/
    │   ├── fridge_tab.dart                ← 냉장고 탭
    │   ├── fridge_screen.dart             ← 단일 스크롤 목록 화면
    │   ├── expiry_preset_management_screen.dart  ← 프리셋 관리 화면
    │   └── _fridge_onboarding.dart        ← 첫 진입 코치마크
    └── widgets/
        ├── storage_form_dialog.dart
        ├── fridge_item_form_dialog.dart   ← 추천 칩 + 기준 품목 선택 버튼
        ├── fridge_item_tile.dart          ← D-Day 뱃지 + 수량 조절
        ├── fridge_group_selector.dart     ← 그룹 선택
        └── expiry_reference_selector_sheet.dart  ← 기준 품목 선택 모달
```
