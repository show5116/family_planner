...
Phase 3  콘솔 등록       🟨 애플 현지화까지 / 구글 AAB 업로드 대기  ← 지금 여기
Phase 4  서버 키·웹훅    ⬜
Phase 5  샌드박스 테스트 ⬜
Phase 6  v1.2.0 릴리즈   ⬜
```

**바로 다음 할 일**: `app-prod-release.aab`를 Play Console 내부 테스트에 업로드
→ 구독 만들기 버튼이 열린다.
...
### 🤖 Play Console
- [ ] **AAB 내부 테스트 업로드** ← 현재 막힌 지점
- [ ] 구독 `family_planner_ad_free_monthly` / 이름 `광고 제거`
- [ ] **기본 요금제** `monthly-autorenew` / 자동갱신 / 1개월 / ₩1,900 / 대한민국
- [ ] 혜택 이름·설명 (애플과 동일 문구)
- [ ] **기본 요금제 활성화 + 구독 활성화** (2단계 모두!)
- [ ] 라이선스 테스터 등록 (일반 Gmail 가능)

> 구글은 **빌드가 한 번이라도 업로드되어야** 구독 상품 생성이 열린다.
> 그래서 AAB 업로드가 선행되어야 한다.
