...
**아이콘 크기는 간격과 달리 raw 값을 허용한다.** 상수가 있으면 쓰되, 없는 크기를 위해 억지로 상수를 만들지 않는다.
...
실제로 쓰이는 크기는 19종으로 흩어져 있고(`18` 120곳, `20` 93곳, `16` 76곳, `14` 45곳, …), 상수 사용은 133곳·raw는 424곳이다. **이건 정리해야 할 부채가 아니라 이 프로젝트가 자리 잡은 방식이다.**

간격(`space*`)은 "기본 여백"이라는 공유 개념이 있어 한 번에 바꿀 수 있지만, 아이콘 크기는 그렇지 않다. `18`을 쓰는 120곳은 더보기 버튼·화살표·상태 아이콘 등 서로 무관한 자리들이라 **우연히 같은 숫자일 뿐 함께 바뀔 일이 없다.** `iconSmall`(16)과 `iconMedium`(24) 사이에 18·20·22를 넣으려 해도 부를 이름이 없는데, 이름이 안 나온다는 건 공유되는 개념이 없다는 뜻이다.

**따라서 새 상수를 추가하지 않는다.** 크기는 주변 요소와의 시각적 균형으로 정하고, 같은 화면·같은 맥락의 기존 코드와 맞춘다.

```dart
// ✅ 상수가 맞으면 상수
Icon(Icons.edit, size: AppSizes.iconSmall)

// ✅ 없는 크기는 raw 값 (주변 코드와 맞춤)
Icon(Icons.more_vert, size: 18)  // routine 위젯의 인라인 아이콘 관행
```
...
**아이콘 크기(§3)와 달리, 여기는 상수를 쓰는 게 맞다.** 값이 4·8·16 세 단계로 수렴하고 "카드는 8, 배지는 4"처럼 역할이 명확하기 때문이다.

> ⚠️ 다만 raw `circular(N)`이 174곳 남아 있고, 이 중 **`circular(4)` 40곳 / `circular(8)` 20곳 / `circular(16)` 2곳은 이미 상수가 있는 값을 raw로 쓴 것**이다(= 고쳐야 할 불일치). 나머지 `12`(31곳) · `20`(30곳) · `2`(29곳) 등은 대응 상수가 없다.
>
> 신규 코드에서 4·8·16·24는 반드시 상수를 쓴다. 그 외 값이 꼭 필요하면 raw를 허용하되 근거를 주석으로 남긴다.
...
- [ ] 아이콘 크기는 상수가 있으면 상수, 없으면 주변 코드와 맞춘 raw 값 (§3)
- [ ] 모서리 4·8·16·24는 `AppSizes.radius*` (§4)
